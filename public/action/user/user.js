$(document).ready(function () {
    ...
    $('#regional-select').on("select2:select", function(e) { 
        ...
        var table = new Table(tableData)
        table.dataTable(AUTH_TOKEN, data)
    });
    ...
    $('#employee_email').keyup(function () {
        ...
        var emailValidation = email.emailValidation(employeeEmail)
    ...
    $(document).on('click', '.input-user', async function(e){
        ...
        var emailValidation = email.emailValidation($('#employee_email').val())
        ...
        if (emailValidation == true || data.employee_email == '') {

            if (data.employee_birth < data.join_date && data.quit_date >= formattedDate || data.employee_birth == '') {
                
                $('#employeeTable').DataTable().destroy()
                var ajax = new Ajax(APP_URL + "api/add-user", data, "POST")
                ajax.ajaxFunction(tableData, "#staticBackdrop")
            }
    ...
    $(document).on('click', '.update-user', function(e){
        ...
        var ajax = new Ajax(APP_URL + "api/update-user/"+ id, data, "PUT")
        ajax.ajaxFunction(tableData, "#modalAddUser")
    ...
    $(document).on('click', '#reset-pass', function(e){
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                var ajax = new Ajax(APP_URL + "api/reset-user-password/"+id, false, "POST")
                ajax.ajaxFunction(tableData)
            }
        })
    ...
    $(document).on('click', '#delete-user', function(e){
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                var ajax = new Ajax(APP_URL + "api/delete-user/"+  id, false, "DELETE")
                ajax.ajaxFunction(tableData)
            }
          })
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                if (userStatus == 'Active') {
                    var ajax = new Ajax(APP_URL + "api/set-status-employee/" + id, data, "POST")
                    ajax.ajaxFunction(tableData)
                }else{
